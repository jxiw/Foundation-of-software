 ...
 *  the TAPL book.
 */
object SimplyTyped1 extends StandardTokenParsers {
  ...
  def subst(t: Term, x: String, s: Term): Term = t match{
      ...
      case Abs(y,type1,t1) if y!=x && (FV(s) contains y) => { val t2 = Abs("x3",type1,rename(t1,y))
        subst(t2,x,s);
      }
  ...
  def typeof(ctx: Context, t: Term): Type = t match {
    ...
    case Pred(t1) =>{
      try{
          if (typeof(ctx,t1) == TypeNat) 
            ...
            throw TypeError(t,"parameter type mismatch")
      }
      catch{
        case TypeError(_,_) =>throw TypeError(t,"parameter type mismatch")
      }
    }
    case Succ(t1) =>{
      try{
    ...
    case If(cond,t1,t2) => {
      try{  
        ...
        if (typeof(ctx,cond) == TypeBool && type1==type2) { 
          return type1
        }
        else{
          throw TypeError(t,"parameter type mismatch: expected "+type1+", found "+type2)
        }
      }
      catch{
        case TypeError(_,_) =>throw TypeError(t,"parameter type mismatch")
    ...
    case Var(x) => {
      try{
        getTypeFromList(ctx,x)
      }
      catch{
        case exception:Throwable=>throw TypeError(t,"parameter type mismatch")
      }
    }
    
    case Abs(x,type1,t2) => { 
      val type2 = typeof((x,type1) +: ctx, t2)
      TypeFun(type1,type2)
    }
    case App(t1,t2) => {
      typeof(ctx,t1) match{
